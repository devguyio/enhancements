...
Because `kmsKeyARN` is a day-1-only knob, two additional spec-level rules on
`HostedClusterSpec` prevent adding or removing `kmsKeyARN` post-creation through
the entire parent chain:

- **Addition guard**: if the new spec has `kmsKeyARN` set all the way through
  `operatorConfiguration.csiDriverConfig.aws.kmsKeyARN`, the old spec must also
  have had it. This prevents adding KMS encryption to an existing cluster.
- **Removal guard**: if the old spec had `kmsKeyARN` set all the way down, the
  new spec must still have `operatorConfiguration` present. Once inside, the
  nested parent guards handle the rest.

These rules are required at the spec level because nested transition rules
(referencing `oldSelf`) are skipped when the parent struct is absent from either
the old or new object. The spec level is the only place where `oldSelf` is
guaranteed on every update. This follows the `infraID`/`clusterID` precedent
...
- Immutability: adding `kmsKeyARN` to a cluster created without it is rejected
- Immutability: removing `operatorConfiguration` while `kmsKeyARN` is set is rejected
